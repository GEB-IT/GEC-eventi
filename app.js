...
function getAgendaParts(day, index) {
  return {
    time: getValue(`orario${index}_d${day}`),
    activity: getActivityValue(day, index)
  };
}
...
function fitAgendaPartsToWidth(time, activity, box, font, boldFont) {
  const size = box.fontSize || 14;
  let fittedTime = String(time || '');
  let fittedActivity = String(activity || '');
  const separator = fittedTime && fittedActivity ? ' – ' : '';

  const measure = () => {
    const timeWidth = fittedTime ? boldFont.widthOfTextAtSize(fittedTime, size) : 0;
    const separatorWidth = separator ? font.widthOfTextAtSize(separator, size) : 0;
    const activityWidth = fittedActivity ? font.widthOfTextAtSize(fittedActivity, size) : 0;
    return timeWidth + separatorWidth + activityWidth;
  };

  while (measure() > box.width && fittedActivity.length > 3) {
    fittedActivity = fittedActivity.slice(0, -2).trim() + '...';
  }

  while (measure() > box.width && fittedTime.length > 3) {
    fittedTime = fittedTime.slice(0, -2).trim() + '...';
  }

  return { fittedTime, fittedActivity, separator, totalWidth: measure() };
}

function drawAgendaText(page, time, activity, box, font, boldFont, color) {
  if ((!time && !activity) || !box) return;
  const size = box.fontSize || 14;
  const { fittedTime, fittedActivity, separator, totalWidth } = fitAgendaPartsToWidth(time, activity, box, font, boldFont);

  let x = box.x;
  if (box.align === 'right') x = box.x + box.width - totalWidth;
  if (box.align === 'center') x = box.x + (box.width - totalWidth) / 2;

  if (fittedTime) {
    page.drawText(fittedTime, { x, y: box.y, size, font: boldFont, color });
    x += boldFont.widthOfTextAtSize(fittedTime, size);
  }

  if (separator) {
    page.drawText(separator, { x, y: box.y, size, font, color });
    x += font.widthOfTextAtSize(separator, size);
  }

  if (fittedActivity) {
    page.drawText(fittedActivity, { x, y: box.y, size, font, color });
  }
}

function drawAgendaLines(page, agendaBoxes, day, font, boldFont, color) {
  agendaBoxes.forEach((box, idx) => {
    const { time, activity } = getAgendaParts(day, idx + 1);
    drawAgendaText(page, time, activity, box, font, boldFont, color);
  });
}

function getCommonRsvpValue() {
  return getValue('data2_d1') || getValue('data2_d2');
}

function drawOneDay(page, font, boldFont, color) {
  const layout = templateConfig.layouts.oneDay;
  drawAlignedText(page, formatItalianDate(getValue('data1_d1')), layout.data1, boldFont, color);
  drawAgendaLines(page, layout.agenda, 1, font, boldFont, color);
  drawAlignedText(page, formatRsvpText(getValue('data2_d1')), layout.data3, boldFont, color);
}

function drawTwoDays(page, font, boldFont, color) {
  const layout = templateConfig.layouts.twoDays;
  drawAlignedText(page, formatItalianDate(getValue('data1_d1')), layout.day1.data1, boldFont, color);
  drawAgendaLines(page, layout.day1.agenda, 1, font, boldFont, color);

  drawAlignedText(page, formatItalianDate(getValue('data1_d2')), layout.day2.data1, boldFont, color);
  drawAgendaLines(page, layout.day2.agenda, 2, font, boldFont, color);

  drawAlignedText(page, formatRsvpText(getCommonRsvpValue()), layout.data3, boldFont, color);
}
...
async function generatePdf() {
  ...
  if (dayCount === 1) {
    drawOneDay(page, font, boldFont, white);
  } else {
    drawTwoDays(page, font, boldFont, white);
  }
